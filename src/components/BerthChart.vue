...
export default class BerthChart extends Vue {
  // $refs!: {
  //   BerthChartParent: HTMLElement;
  //   BerthChartSvg: HTMLElement;
  // };
  @Ref("BerthChartParent") readonly BerthChartParent!: HTMLElement;
  @Ref("BerthChartSvg") readonly BerthChartSvg!: HTMLElement;
  // Parameter  Drawing
  private fw = 4; //Fact for Chart Width
  private vw = 0; // Max Chart Width
  private rowHeight = 200; //is Fixed
  private berthH = 60; //Berth Height
  berthBreaksWidth = 5;

  private berths: Array<any> = [];
  private bolards: Array<any> = [];
  private layers: Array<Layer> = [];
  private daysCaptions: Array<any> = [];
  private grips: Array<any> = [];
  private berthBreaks: Array<any> = [];
  private noneWorkingZone: Array<any> = [];
  //Scroll Change
  private scrollTop = 0;
  private scrollLeft = 0;
  private viewWidth = 0;

  private leftOffset = 90;

  //Zoom
  private zoom = 1;
  //Pan
  private panzoom: any;
  private panzoom_grip: any;
  private panzoom_parent: any;
  private gripMoveing = false;
  private selectObject = null;

  private pathConfilicts = [];
  startDate: Date;
  canMoveX = true;
  canMoveY = true;
  ...
  mousewheel(e) {
    ...
    if (dir == -1 && this.zoom < 20) {
      this.zoom += 0.3;
    }
    if (dir == 1 && this.zoom > 1) {
      this.zoom += -0.3;
    }
  ...
  mounted() {
    this.init();
    // this.layers = this.fakeGroup();
    //this.setPanZoom_Parent();
    // this.grips = this.fakeGroup().map((r) => {
    //   r["link"] = r.id;
    //   r.id = r.id + "_grip";
    //   r["objectType"] = "grip";
    //   r.x = r.x + r.w / 2;
  ...
  setPanZoom(layer) {
    const elem = this.BerthChartSvg.querySelector(
      "#" + layer.Id
    ) as HTMLElement;
  ...
  onChartScroll(e) {
    const parent = e.srcElement as HTMLElement;
